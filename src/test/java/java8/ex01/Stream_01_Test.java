...
public class Stream_01_Test {
    ...
    public void test_stream_anyMatch() throws Exception {
        ...
        Boolean result1 = pizzas.stream().anyMatch(p -> p.getPrice() > 1299);

        // TODO valider si au moins une pizza à un prix >= 2000
        Boolean result2 = pizzas.stream().anyMatch(p -> p.getPrice() > 1999);
    ...
    public void test_stream_allMatch() throws Exception {
        ...
        Boolean result1 = pizzas.stream().allMatch(p -> p.getPrice() > 1299);

        // TODO valider que toutes les pizzas ont un prix >= 900
        Boolean result2 = pizzas.stream().allMatch(p -> p.getPrice() > 899);
    ...
    public void test_stream_noneMatch() throws Exception {
        ...
        Boolean result1 = pizzas.stream().noneMatch(p -> p.getPrice() > 1999);
    ...
    public void test_stream_filter_and_match() throws Exception {
        ...
        List<Order> result = orders.stream()
        		...
        		.filter(o -> o.getPizzas().stream().anyMatch(p -> p.getPrice() > 1299))
    ...
    public void test_stream_min() throws Exception {
        ...
        Optional<Pizza> result = pizzas.stream().filter(p -> p.getPrice() > 949).min(Comparator.comparing(Pizza::getPrice));
